...
import syllabusData  from "../../data/syllabus"; // ✅ use named import
...
export default function ClassPage() {
  ...
  // ✅ Get syllabus for current class + subject
  const syllabus =
    decodedClassName && selectedSubject
      ? syllabusData[decodedClassName]?.[selectedSubject] || []
      : [];

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 via-purple-50 to-pink-100 px-4 sm:px-6 py-[100px] flex gap-6">
      {/* Sidebar Syllabus */}
      <div className="w-80 h-95 mt-45 bg-white/80 backdrop-blur-md border-2 border-purple-200 rounded-2xl p-6 shadow-md  sticky top-28">
        <h3 className="text-xl font-bold text-purple-700 mb-4">📘Syllabus of {selectedSubject}</h3>
        {selectedSubject ? (
          syllabus.length > 0 ? (
            ...
            <p className="text-gray-400 italic">No syllabus found</p>
          )
        ) : (
          <p className="text-gray-400 italic">Select a subject to view syllabus</p>
        )}
      </div>
